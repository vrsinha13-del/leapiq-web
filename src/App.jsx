...
import { selectNextQuestion, updateRecord, sessionEndMessage, strengthSummary, fullTopicBreakdown, DIFF_LABEL, DIFF_TIME } from './lib/engine';
...
function showToast(msg, bg='#1e1b4b') {
  ...
  t.textContent = msg;
  Object.assign(t.style, { position:'fixed', top:'70px', left:'50%', transform:'translateX(-50%)', background:bg, color:'#fff', borderRadius:'99px', padding:'10px 22px', fontSize:'15px', fontWeight:'700', zIndex:'9999', pointerEvents:'none', animation:'fadeToast 2s ease-in-out forwards', whiteSpace:'nowrap' });
...
function HomeScreen({ setScreen, startSubject }) {
  ...
  function sessForSubj(sid) { return sessionHistory.filter(h => h.subject === sid).length; }
  function avgForSubj(sid) {
    const ks = Object.keys(topicRecords).filter(k => k.startsWith(sid+'_'));
    if (!ks.length) return null;
    const sc = ks.map(k => { const r=topicRecords[k]; return r.answered>=3?Math.round((r.correct/r.answered)*100):null; }).filter(s=>s!==null);
    ...
    <div>
      ...
      <div style={{ background:'linear-gradient(135deg,#1e1b4b 0%,#4338ca 60%,#7c3aed 100%)', padding:'0 0 52px', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'18px' }}>
          <div>
            ...
            <div style={{ fontSize:11, color:'rgba(255,255,255,0.55)', marginTop:1 }}>Adaptive Learning · Grades 6–10</div>
          </div>
      ...
      <div style={{ padding:'0 14px', marginTop:-20 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:11 }}>
          {SUBJECTS.map((s,i) => {
            const avg = avgForSubj(s.id), sess = sessForSubj(s.id);
            return (
...
function PracticeScreen({ setScreen, subject, subj, onEnd, onLoginRequired }) {
  ...
  function handleAnswer(opt) {
    ...
    setAnswered(true);
    setIsLate(late);
    recordAnswer(subject, current.question.topic, correct, late, current.question.id);
    setQCount(c => c + 1);
    showToast(correct ? OK_MSGS[Math.floor(Math.random()*OK_MSGS.length)] : WRONG_MSGS[Math.floor(Math.random()*WRONG_MSGS.length)]);
  }
    ...
    <div style={{ minHeight:'100dvh', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ background:subj?.color||'#4338ca', padding:'14px 16px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:10 }}>
          ...
          <div style={{ flex:1, textAlign:'center', fontSize:12, color:'rgba(255,255,255,0.85)', fontWeight:600 }}>{subj?.short} · {qCount} answered</div>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'16px 14px 100px' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:13 }}>
          ...
          <span style={{ fontSize:11, fontWeight:700, background:subj?.light, color:subj?.color, borderRadius:99, padding:'3px 11px' }}>{DIFF_LABEL[di]}</span>
        </div>
        <div className="q-card">{q.q}</div>
        <div style={{ display:'flex', flexDirection:'column', gap:10, marginBottom:14 }}>
          {q.opts.map((opt, i) => {
            const isCorrect = opt === q.ans, isSel = opt === selected;
            ...
            <div style={{ borderRadius:13, padding:14, background:selected===q.ans?'#dcfce7':'#fee2e2', display:'flex', gap:9, alignItems:'flex-start', marginBottom:12 }}>
              ...
              <div>
                <div style={{ fontWeight:700, fontSize:14, color:selected===q.ans?'#15803d':'#dc2626' }}>
                  {selected===q.ans ? (isLate?'Correct! (Take your time next time ⏱)':'Correct!') : `Answer: ${q.ans}`}
                </div>
                {q.exp && <div style={{ fontSize:12, color:'#4b5563', marginTop:3, lineHeight:1.5 }}>{q.exp}</div>}
              </div>
            </div>
            <button className="primary-btn" style={{ background:subj?.color }} onClick={handleNext}>Next Question →</button>
          </>
        )}
      </div>
    ...
    <div style={{ minHeight:'100dvh', background:'#f0effe', padding:'32px 16px 40px', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center' }}>
      <div className="info-card" style={{ maxWidth:420, width:'100%', textAlign:'center', marginBottom:16 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
          {[['questionsAnswered',questionsAnswered,'questions answered'],[' subject',subj?.short,'subject practised']].map(([,v,l],i) => (
            <div key={i} style={{ background:'#f8f7ff', borderRadius:11, padding:'14px 8px' }}>
...
function SignupScreen({ setScreen, goHome }) {
  ...
  function handleSave(e) {
    ...
    const pin = genPin();
    useStore.getState().login({ id: Date.now()+'', name: name.trim(), email: email.trim().toLowerCase(), grade: parseInt(grade), parentPin: pin, parentPinChanged: false });
  ...
  return (
    <div style={{ minHeight:'100dvh', display:'flex', flexDirection:'column' }}>
      <div style={{ background:'linear-gradient(135deg,#1e1b4b,#4338ca)', padding:'32px 20px 48px', textAlign:'center' }}>
        ...
        <div style={{ color:'rgba(255,255,255,0.75)', fontSize:14 }}>Free forever · Track your progress</div>
      </div>
  ...
  return (
    <div style={{ minHeight:'100dvh', display:'flex', flexDirection:'column' }}>
      <div style={{ background:'linear-gradient(135deg,#059669,#0D9488)', padding:'32px 20px 48px', textAlign:'center' }}>
        ...
        <div style={{ fontFamily:"'Syne',system-ui", fontSize:24, fontWeight:800, color:'#fff', marginBottom:6 }}>Welcome to Leap IQ, {user?.name?.split(' ')[0]}!</div>
      ...
      <div style={{ flex:1, padding:'24px 20px', background:'#fff', borderTopLeftRadius:22, borderTopRightRadius:22, marginTop:-18 }}>
        <div style={{ background:'#f0fdf4', border:'2px solid #86efac', borderRadius:16, padding:20, textAlign:'center', marginBottom:20 }}>
          <div style={{ fontSize:13, fontWeight:700, color:'#15803d', marginBottom:4 }}>🔑 Your Parent Access PIN</div>
          ...
          <div style={{ fontSize:11, color:'#15803d', marginTop:10, fontWeight:600, background:'#dcfce7', borderRadius:8, padding:'8px 12px' }}>
            ⚠ Your parent will set a private PIN on their first login. After that you cannot access the Parent Dashboard.
          </div>
        </div>
...
function ParentLogin({ setScreen, goHome }) {
  ...
  function handleLogin(e) {
    ...
    if (!user || user.email.toLowerCase() !== email.trim().toLowerCase()) { setErr('No student found with this email.'); return; }
    ...
    <div style={{ minHeight:'100dvh', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ flex:1, padding:'24px 20px', background:'#fff', borderTopLeftRadius:22, borderTopRightRadius:22, marginTop:-18 }}>
        <form onSubmit={handleLogin} style={{ display:'flex', flexDirection:'column' }}>
          ...
          <input className="field" type="tel" maxLength={6} value={pin} onChange={e=>setPin(e.target.value.replace(/\D/,'').slice(0,6))} placeholder="••••••" style={{ letterSpacing:8, fontSize:22, textAlign:'center' }}/>
          {err && <div className="error-box">{err}</div>}
...
function ParentChangePin({ setScreen, goHome }) {
  ...
  function handleSave(e) {
    ...
    if (newPin !== confPin) { setErr('PINs do not match. Please try again.'); return; }
  ...
  return (
    <div style={{ minHeight:'100dvh', display:'flex', flexDirection:'column' }}>
      <div style={{ background:'linear-gradient(135deg,#1e1b4b,#312e81)', padding:'28px 20px 44px', textAlign:'center' }}>
        ...
        <div style={{ color:'rgba(255,255,255,0.75)', fontSize:13, lineHeight:1.6 }}>Create a new 6-digit PIN only you will know. The student cannot access the dashboard without it.</div>
      </div>
      <div style={{ flex:1, padding:'24px 20px', background:'#fff', borderTopLeftRadius:22, borderTopRightRadius:22, marginTop:-18 }}>
        <form onSubmit={handleSave} style={{ display:'flex', flexDirection:'column' }}>
          <label className="lbl">Choose your new PIN:</label>
          <input className="field" type="tel" maxLength={6} value={newPin} onChange={e=>setNewPin(e.target.value.replace(/\D/,'').slice(0,6))} placeholder="••••••" style={{ letterSpacing:8, fontSize:22, textAlign:'center' }}/>
          <label className="lbl">Confirm your new PIN:</label>
          <input className="field" type="tel" maxLength={6} value={confPin} onChange={e=>setConfPin(e.target.value.replace(/\D/,'').slice(0,6))} placeholder="••••••" style={{ letterSpacing:8, fontSize:22, textAlign:'center' }}/>
          {err && <div className="error-box">{err}</div>}
          <div style={{ background:'#EEF2FF', borderRadius:12, padding:'12px 14px', marginBottom:16, fontSize:12, color:'#4338ca', fontWeight:600, lineHeight:1.6 }}>
            ⚠ Once set, the student's original PIN will stop working. Only you can access the Parent Dashboard.
          </div>
  ...
  return (
    <div style={{ minHeight:'100dvh' }}>
      <div style={{ background:'linear-gradient(135deg,#1e1b4b,#312e81)', padding:'20px 18px 40px' }}>
        ...
        <div style={{ color:'rgba(255,255,255,0.7)', fontSize:13 }}>{user?.name||'Student'} · Grade {user?.grade||'—'} · {sessionHistory.length} sessions</div>
              ...
              {breakdown.length > 0 ? (
                <>
                  <div style={{ display:'flex', flexDirection:'column', gap:8, marginBottom:12 }}>
                    {breakdown.map(t => (
                      <div key={t.topic} style={{ display:'flex', alignItems:'center', gap:8 }}>
                        ...
                        <span style={{ fontSize:11, fontWeight:700, color:t.needsWork?'#dc2626':subj.color, width:34, textAlign:'right' }}>{t.score===null?'—':`${t.score}%`}</span>
                  ...
                  {breakdown.filter(t=>t.needsWork).length > 0
                    ? <div style={{ background:'#fef3c7', borderRadius:10, padding:'10px 14px', fontSize:13, color:'#92400e', fontWeight:600 }}>⚠ Needs attention: {breakdown.filter(t=>t.needsWork).map(t=>t.label).join(', ')}</div>
                    : <div style={{ background:'#dcfce7', borderRadius:10, padding:'10px 14px', fontSize:13, color:'#15803d', fontWeight:600 }}>✅ All topics looking strong!</div>
                  }
                </>
              ) : (
